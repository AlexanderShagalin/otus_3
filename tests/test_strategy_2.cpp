...
class SecondRetryHandlerMock : public IHandler
{
public:
     MOCK_METHOD(void, handle, (ICommandPtr command));
};

TEST(Strategy_2, retry_handled)
{
...
TEST(Strategy_2, retry_not_handled)
{
    auto commandQueue = std::make_shared<CommandQueue>(std::make_shared<ExceptionManager>());
...
TEST(Strategy_2, secondretry_handled)
{
    auto commandQueue = std::make_shared<CommandQueue>(std::make_shared<ExceptionManager>());
    auto logger = std::make_shared<LoggerMock>();
    auto command = std::make_shared<MoveForward>(std::make_shared<NonMovableObject>());
    auto retryHandler = std::make_shared<RetryHandler>(commandQueue);
    auto secondRetryHandler = std::make_shared<SecondRetryHandlerMock>();
    auto logHandler = std::make_shared<LogHandlerMock>();

    ExceptionManager::registerHandler(command, CommandException(), std::dynamic_pointer_cast<IHandler>(retryHandler));
    ExceptionManager::registerHandler(std::make_shared<RetryCommand>(nullptr), RetryException(), std::dynamic_pointer_cast<IHandler>(secondRetryHandler));

    commandQueue->push_back(command);

    EXPECT_CALL(*secondRetryHandler, handle(_));
...
TEST(Strategy_2, secondretry_not_handled)
{
    auto commandQueue = std::make_shared<CommandQueue>(std::make_shared<ExceptionManager>());
    auto logger = std::make_shared<LoggerMock>();
    auto command = std::make_shared<MoveForward>(std::make_shared<MovableObject>());
    ...
    EXPECT_CALL(*secondRetryHandler, handle(_)).Times(0);
...
TEST(Strategy_2, logger_handled)
{
    auto commandQueue = std::make_shared<CommandQueue>(std::make_shared<ExceptionManager>());
    auto logger = std::make_shared<LoggerMock>();
    auto command = std::make_shared<MoveForward>(std::make_shared<NonMovableObject>());
    auto retryHandler = std::make_shared<RetryHandler>(commandQueue);
    auto secondRetryHandler = std::make_shared<SecondRetryHandler>(commandQueue);
    auto logHandler = std::make_shared<LogHandlerMock>();

    ExceptionManager::registerHandler(command, CommandException(), std::dynamic_pointer_cast<IHandler>(retryHandler));
    ExceptionManager::registerHandler(std::make_shared<RetryCommand>(nullptr), RetryException(), std::dynamic_pointer_cast<IHandler>(secondRetryHandler));
    ExceptionManager::registerHandler(std::make_shared<SecondRetryCommand>(nullptr), SecondRetryException(), std::dynamic_pointer_cast<IHandler>(logHandler));
...
TEST(Strategy_2, logger_not_handled)
{
    auto commandQueue = std::make_shared<CommandQueue>(std::make_shared<ExceptionManager>());
    auto logger = std::make_shared<LoggerMock>();
    auto command = std::make_shared<MoveForward>(std::make_shared<MovableObject>());
...
TEST(Strategy_2, logger_write_log)
{
    auto commandQueue = std::make_shared<CommandQueue>(std::make_shared<ExceptionManager>());
    auto logger = std::make_shared<LoggerMock>();
    auto command = std::make_shared<MoveForward>(std::make_shared<NonMovableObject>());
